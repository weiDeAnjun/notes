...
多线程，每个线程停在什么时候都是未知的，目前发现的2类问题是
        ...
        static class Card {
            ...
            public static int getMoney() {
                synchronized (Card.class) {
                    ...
                    money -= 1000;
                    //这个打印操作如果不锁在这里，可能导致延迟输出，比如2取完钱后还剩8000，刚要打印，换到线程1了，这时候1又取走钱，还剩7000，虽然线程2也同步了余额为7000，但是打印还没结束，导致输出8000
                    System.out.println(Thread.currentThread().getName() + " 扣钱后余额：" + money);
                    return money;
                }
            }
...
public class Main {
    ...
    static class A extends Thread {
        ...
        public void run() {
            while (flag) {
                // 调用扣钱方法，返回是否扣钱成功
                int[] result = Card.deductMoney();
                int money = result[0];
                boolean success = result[1] == 1;
                ...
                if (success) {
                    ...
                    // 扣钱失败（余额不足），停止线程
                    flag = false;
                    System.out.println(Thread.currentThread().getName() + " 余额不足，停止扣钱");
                }

                try {
                    Thread.sleep(100); // 减缓执行速度，避免线程抢跑
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
    ...
    static class Card {
        ...
        // 同步块：返回数组[当前余额, 是否成功扣钱(1=成功,0=失败)]
        public static int[] deductMoney() {
            synchronized (Card.class) {
                if (money < 1000) { // 余额不足1000，扣钱失败
                    return new int[]{money, 0};
                }
                // 扣钱成功
                money -= 1000;
                return new int[]{money, 1};
            }
        }
    }
}

/*原本线程终止条件是返回余额，判断余额是否为空，决定是否打印，现在改为余额为空返回flase
线程终止条件是boolean，如果2个线程都进入打印阶段，*/


int money = Card.getMoney(); // 扣钱并返回当前余额
if (money == 0) {
    flag = false; // 余额为0时停止
}

但此时的money是扣钱后的余额，而线程的while(flag)循环会再执行一次（因为flag的修改是在本次循环末尾）。例如：
当money=1000时，线程 1 调用getMoney()扣钱后返回0，设置flag=false，本次循环结束。
但线程 0 可能在此之前已经进入getMoney()方法（由于同步块的原子性，此时money还剩1000），扣钱后返回0，但线程 0 的flag仍为true，会继续执行一次循环（实际扣钱时money已为0，但同步块内会允许扣最后一次）。
同步块仅保证扣钱原子性，但无法阻止线程 “抢跑”
Card.getMoney()的同步块确保了扣钱操作的原子性，但无法阻止多个线程在余额接近 0 时 “抢着” 进入同步块。例如：
余额剩1000时，线程 1 和线程 0 可能先后进入同步块：
线程 1 先扣钱，余额变为0，返回0。
线程 0 进入同步块时，虽然money已为0，但同步块内的逻辑是if(money <=0) return 0，但此时线程 0 的flag仍为true，会执行一次System.out打印（显示扣钱后余额0）。
```
...
![image-20250801104949446](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801104949446.png)



IO是以java程序为参照，进入程序的是输入，从程序出去的是输出



目录、文件创建方法(在java编程中，目录也被当做文件)

![image-20250801105346374](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801105346374.png)
...
包装流：修饰器设计模式，节点流的封装，提供便捷输入输出方法，消除不同节点流差异，不与数据源直接相连



![image-20250801111546311](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801111546311.png)

由以上基础类，衍生对文件、缓冲、对象操作的IO流👇

字节流、字符流各自实现了对文件、缓冲字节、对象字节的实现

![image-20250801111903231](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801111903231.png)

使用后记得.close()关闭



![image-20250801112954708](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801112954708.png)
...
![image-20250801114104170](D:\01\技术\感获\md文档\JavaSE.assets\image-20250801114104170.png)
